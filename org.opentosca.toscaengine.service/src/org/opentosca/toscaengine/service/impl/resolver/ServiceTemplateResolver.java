...
public class ServiceTemplateResolver extends GenericResolver {
	...
	public boolean resolve(Definitions definitions, CSARID csarID) {
		...
		referenceMapper.storeDocumentIntoReferenceMapper(definitionsID, definitionsDocument);
		...
		for (TExtensibleElements element : definitions.getServiceTemplateOrNodeTypeOrNodeTypeImplementation()) {
			...
			if (element instanceof TServiceTemplate) {
				...
				referenceMapper.storeJAXBObjectIntoToscaReferenceMapper(serviceTemplateID, serviceTemplate);
				
				// resolve the SubstitutableNodeType
				if (serviceTemplate.getSubstitutableNodeType() != null) {
					errorOccurred = errorOccurred || !referenceMapper.searchToscaElementByQNameWithName(serviceTemplate.getSubstitutableNodeType(), ElementNamesEnum.NODETYPE);
				}
				
				// resolve the other data objects
				errorOccurred = errorOccurred || resolveTags(serviceTemplate.getTags(), targetNamespace);
				errorOccurred = errorOccurred || resolveBoundaryDefinitions(serviceTemplate, definitions.getTargetNamespace(), csarID);
				errorOccurred = errorOccurred || resolveTopologyTemplate(serviceTemplate.getTopologyTemplate(), serviceTemplateID, csarID);
				errorOccurred = errorOccurred || resolvePlans(serviceTemplate.getPlans(), definitionsID, serviceTemplateID, csarID);
			}
		}
			...
			if (boundaryDefinitions.getInterfaces() != null) {
				...
				for (TExportedInterface iface : boundaryDefinitions.getInterfaces().getInterface()) {
					...
					if (iface.getOperation().size() > 0) {
						referenceMapper.storeExportedInterface(csarID, serviceTemplateID, iface);
					}
	 ...
	 * @return true if an error occurred, false if not
	 */
	private boolean resolveTopologyTemplate(TTopologyTemplate topologyTemplate, QName serviceTemplateID, CSARID csarID) {
		
		String targetNamespace = serviceTemplateID.getNamespaceURI();
		...
		if (topologyTemplate.getNodeTemplateOrRelationshipTemplate().size() > 0) {
			for (TEntityTemplate template : topologyTemplate.getNodeTemplateOrRelationshipTemplate()) {
				...
				if (template instanceof TNodeTemplate) {
					...
					referenceMapper.storeNodeTemplateIDForServiceTemplateAndCSAR(csarID, serviceTemplateID, nodeTemplate.getId());
					
					QName nodeTemplateID = new QName(targetNamespace, nodeTemplate.getId());
					referenceMapper.storeJAXBObjectIntoToscaReferenceMapper(nodeTemplateID, nodeTemplate);
					
					// resolve the NodeType
					if ((nodeTemplate.getType() != null) && !nodeTemplate.getType().toString().equals("")) {
						errorOccurred = errorOccurred || !referenceMapper.searchToscaElementByQNameWithName(nodeTemplate.getType(), ElementNamesEnum.NODETYPE);
					}
					...
					if (nodeTemplate.getRequirements() != null) {
						for (TRequirement requirement : nodeTemplate.getRequirements().getRequirement()) {
							referenceMapper.storeJAXBObjectIntoToscaReferenceMapper(new QName(targetNamespace, requirement.getId()), requirement);
							errorOccurred = errorOccurred || !referenceMapper.searchToscaElementByQNameWithName(requirement.getType(), ElementNamesEnum.REQUIREMENTTYPE);
						}
					}
					...
					if (nodeTemplate.getCapabilities() != null) {
						for (TCapability capability : nodeTemplate.getCapabilities().getCapability()) {
							referenceMapper.storeJAXBObjectIntoToscaReferenceMapper(new QName(targetNamespace, capability.getId()), capability);
							errorOccurred = errorOccurred || !referenceMapper.searchToscaElementByQNameWithName(capability.getType(), ElementNamesEnum.CAPABILITYTYPE);
						}
					}
					...
					if (nodeTemplate.getPolicies() != null) {
						for (TPolicy policy : nodeTemplate.getPolicies().getPolicy()) {
							referenceMapper.storeJAXBObjectIntoToscaReferenceMapper(new QName(targetNamespace, policy.getName()), policy);
							errorOccurred = errorOccurred || !referenceMapper.searchToscaElementByQNameWithName(policy.getPolicyType(), ElementNamesEnum.POLICYTYPE);
							errorOccurred = errorOccurred || !referenceMapper.searchToscaElementByQNameWithID(policy.getPolicyRef());
						}
					}
					...
					if ((nodeTemplate.getDeploymentArtifacts() != null) && (nodeTemplate.getDeploymentArtifacts().getDeploymentArtifact().size() > 0)) {
						for (TDeploymentArtifact deploymentArtifact : nodeTemplate.getDeploymentArtifacts().getDeploymentArtifact()) {
							errorOccurred = errorOccurred || new DeploymentArtifactResolver(referenceMapper).resolve(deploymentArtifact, targetNamespace);
						}
					}
				} else
					...
					if (template instanceof TRelationshipTemplate) {
						...
						if ((relationshipTemplate.getType() != null) && !relationshipTemplate.getType().toString().equals("")) {
							errorOccurred = errorOccurred || !referenceMapper.searchToscaElementByQNameWithName(relationshipTemplate.getType(), ElementNamesEnum.RELATIONSHIPTYPE);
						}
						...
						if ((relationshipTemplate.getSourceElement() != null) && (relationshipTemplate.getSourceElement().getRef() != null)) {
							errorOccurred = errorOccurred || !referenceMapper.searchElementViaIDREF(relationshipTemplate.getSourceElement().getRef(), targetNamespace);
						}
						
						// TargetElement
						if ((relationshipTemplate.getTargetElement() != null) && (relationshipTemplate.getTargetElement().getRef() != null)) {
							errorOccurred = errorOccurred || !referenceMapper.searchElementViaIDREF(relationshipTemplate.getTargetElement().getRef(), targetNamespace);
						}
